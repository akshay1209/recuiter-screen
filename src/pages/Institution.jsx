...
export default function Instituition() {
  ...
  const handleViewDetails = (college, driveDate) => {
    setDetails({
      college,
      driveDate,
      jobsCount: 10, // Replace with actual data
      studentsApplied: 50, // Replace with actual data
    });
  ...
  return (
      ...
      <Card className="flex-1 ">
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-2 sm:space-y-0 pb-6">
          ...
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 w-full sm:w-auto">
            ...
            <button
            className="bg-purple-500 px-5 py-2 rounded-xl text-white font-semibold"
              ...
              <CardContent className="p-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4">
                  <div className="flex items-center mb-2 sm:mb-0">
                    <div>
                      <h3 className="font-semibold text-lg">{drive.college}</h3>
                    </div>
                  </div>
                  <Button
                    variant="secondary"
                    className="text-purple-600 bg-purple-100 hover:text-purple-800 hove:bg-purple-50"
                    onClick={() => handleViewDetails(drive.college, drive.driveDate)}
                  >
                    View Details
                  </Button>
                </div>
    ...
    {isDialogOpen && <AddJD onClose={() => setIsDialogOpen(false)} />}
          {/* Open the ViewDetails dialog if isViewDetailsOpen is true */}
      {IsViewDetailsOpen && details && (
        <ViewDetails
          onClose={() => setIsViewDetailsOpen(false)}
          setI
          college={details.college}
          jobsCount={details.jobsCount}
          studentsApplied={details.studentsApplied}
        />
      )}
    </>    
  );
